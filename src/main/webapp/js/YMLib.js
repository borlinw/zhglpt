...
function loadDist(id, dwbm) {
	$('#' + id)
			.combotree(
					{
						checkbox : true,
						url : '/zhglpt/xtgl/selAllXzqh.do?yhdw=' + dwbm,
						onBeforeExpand : function(node, param) {
							$('#' + id).combotree("tree").tree('options').url = "/zhglpt/xtgl/selAllXzqh2.do?yhdw="
									+ node.id;
						},
	...
	$('#' + id).combotree(
			{
				...
				multiple:true,
				async:false,
				url : '/zhglpt/gcgl/selAllXzqh1.do?yhdw=' + dwbm,
	...
	$('#' + id).combotree(
			{
				...
				multiple:true,
				async:false,
				url : '/zhglpt/gcgl/selAllUnit1.do?yhdw=' + dwbm,
				onLoadSuccess : function (node){
					$('#' + id).combotree('setValue', dwbm);
				}
		});
...
function loadUnit(id, dwbm) {
	$('#' + id).combotree({
		checkbox : false,
		url : '/zhglpt/xtgl/selAllBm.do?yhdw=' + dwbm,
		onBeforeExpand : function(node, param) {
			$('#' + id).combotree("tree").tree('options').url = "/zhglpt/xtgl/selAllBm2.do?yhdw="
					+ node.id;
		},
		onSelect : function(node) {
			YMLib.Var.DistName = node.text
...
function loadBmbm(id, name) {
	$('#' + id).combobox({
		url : '/zhglpt/xtgl/getBmbmTreeByName2.do?yhm='
...
function loadBmbm2(id, name) {
	$.ajax({
		type:'post',
		async:false,
		url:'/zhglpt/xtgl/getBmbmTreeByName2.do',
...
function locationQl(_roadbm,_roadpos){
	...
	YMLib.Var.bm=data;
	YMLib.Var.type="2";
	YMLib.UI.createWindow('locationXm','项目定位','/zhglpt/openlayers.html','app_add',800,500);
...
function locationXm(xmbm,jdbs){
	$.ajax({
		type:'post',
		url:'/zhglpt/qqgl/selectlxbyxmid.do',
...
function createMenunew(id){
	$("#tree"+id).tree({
        //data : treeData,
		url:'/zhglpt/xtgl/createMenu.do?parent='+id,
        lines : true,
...
function getQxfromSession(qx){
	...
	$.ajax({
		...
		dateType:'json',
		async:false,
		url:'/zhglpt/xtgl/getQxfromSession.do',
		success:function(msg){
			qxstrng=msg;
		}
	});
